...
- **BSUIDs can regenerate.** Per [Meta's docs](https://developers.facebook.com/documentation/business-messaging/whatsapp/business-scoped-user-ids), a customer's BSUID changes if they change their phone number, and Meta fires a "system messages" webhook when that happens. Nothing in this codebase listens for it — a regenerated BSUID would silently orphan that customer's conversation history and mute state under the old id, with no alert that it happened. Not built; would need its own design (merge history to the new id? just alert staff?) rather than a quick fix.
- **Manual D1 queries against `sender_id` must quote the value.** A phone number like `601155241769` happens to parse as a numeric literal even unquoted; a BSUID like `MY.1111528448110368` does not — SQLite reads the dot as `table.column` and throws a syntax error. Always `WHERE sender_id = 'MY.1111528448110368'`, quoted, when querying by hand.
